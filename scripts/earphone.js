...
function appendData(data){
    document.querySelector(".smallProducts").innerHTML = null;
    data.forEach(function(ele){
        ...
        // image.style.hover = "image.src=ele.image2"
       ...
       div.append(image, title,  subtitle, price)
...
if(select == "LTH") {
    ProductsArray.sort(function(a,b){
        ...
        var x = a.price.trim().split(' ')[2]
         var y = x.split(',').map(Number).join('')

         var xx = b.price.trim().split(' ')[2]
         var yy = xx.split(',').map(Number).join('')
